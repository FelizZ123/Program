...
void loop() {
  ...
  if (Status_Sw1 == 0) {
    ...
    if((DataRead[12])<10)
    {
      DataRead_12 = String(g)+String(DataRead[12]);
    }
    else
    {
      DataRead_12 = String(DataRead[12]);
    }
    Serial.print("dataread12 = ");
    Serial.println(DataRead_12);
    String Buffer_2 = String(DataRead[0]) + String(DataRead[3]) + String(DataRead[4]) + String(DataRead[5]) + String(DataRead[7]) + String(DataRead[8]) + String(DataRead[9]) + String(DataRead[11]) + DataRead_12 + String(DataRead[13]) +  String(DataRead[14]) + String(DataRead[15]);
